...
class Cells extends GraphicalElement {
    ...
    updateData (data, key) {
        ...
        this.selection = this.group.selectAll('rect').data(data, key).enter().append('rect');
    }
...
class Title extends GraphicalElement {
    ...
    constructor (svg, id, text, fontSize) {
        super(svg, id);
        this.text = text;
        this.selection = this.group.append('text').attr('class', 'title').style('font-size', fontSize).text(this.text);
    }
